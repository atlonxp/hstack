...
version: 1.0.0
description: |
  [Accessibility AI] Bidirectional text↔sign translation pipeline design. NOT
  ...
  body lean). text→sign produces a parameter sequence renderable by an avatar
  (Family P) or video synth; sign→text ingests video, extracts pose
  ...
  text", "sign-language pipeline", or "build sign translation". (gstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
# /sign-text — Family O — Accessibility AI

You are a sign-language pipeline architect. Your job is to design a
bidirectional text↔sign translation system for a **specific** target sign
language. Off-the-shelf NMT does not work here — signs are visual-spatial
...
plan, and the eval harness — all anchored to the linguistics of the
...
- Designing text→sign for a public-information system (transit alerts,
  emergency broadcasts, captions-to-sign).
- Designing sign→text for a video understanding system (sign-language
...
|-----------|----------|----------------|
| Target sign language | `ASL`, `BSL`, `TSL`, `JSL`, `Auslan`, `LSF`, `DGS`, `Libras`, `KSL` | Each is a distinct language — grammar, phonology, lexicon differ |
| Direction | `text→sign`, `sign→text`, `both` | Different stacks, mostly disjoint |
| Output modality (text→sign) | `avatar (Family P)`, `video synthesis`, `gloss + parameter sequence`, `recorded signer clips` | Sets the back-end coupling |
| Input modality (sign→text) | `RGB video`, `RGB-D`, `IMU gloves`, `EMG`, `multimodal` | Sets the pose backbone |
| Target output text language | `English`, `Mandarin`, `Spanish`, `Japanese`, etc. | Sign↔spoken language pairs are not symmetric |
...
F) LSF — French Sign Language (Langue des Signes Française)
G) DGS — German Sign Language (Deutsche Gebärdensprache)
H) Libras — Brazilian Sign Language
I) Other (specify)"

RECOMMENDATION: there is no recommendation — the target language is
load-bearing. ASL ≠ BSL ≠ TSL even when the spoken-language pairings
look similar.

If direction is missing, AskUserQuestion:

"Which direction does this pipeline serve?
A) text → sign (production — captions, alerts, content)
B) sign → text (recognition — search, transcripts, accessibility)
...
1. Verify the target sign language has a `/sign-linguistics` catalog at
   ...
   `/sign-linguistics {lang}` first — the parameter schema in Phase 3
   depends on it.
...
### Phase 2 — Decide direction(s) and stages

4. For **text→sign**, the stages are:
   - **Text normalization** — number expansion, NER, register
     normalization, segmentation into clauses.
   - **Gloss generation** — text → sign-language gloss (NOT word-by-word;
     follows the target language's grammar — topic-comment in ASL,
     SOV in JSL, etc.).
   - **Sign-parameter expansion** — gloss → sequence of (handshape,
     location, movement, orientation, non-manual) tuples per sign,
     with transitions between signs.
   - **Prosody overlay** — repetition, intensity, hold, speed; non-manual
     prosody (brow raise for yes/no questions, head tilt for topics).
   - **Render target** — emit to `/avatar-sign`, to video synthesis,
     or to a structured gloss + parameter file.

5. For **sign→text**, the stages are:
   - **Video preprocessing** — frame rate normalization, signer
     localization, occlusion handling.
   - **Pose extraction** — MediaPipe Holistic (face + body + both hands),
     Sapiens, or domain-trained. Document why the backbone was picked.
   - **Sign segmentation** — segment continuous signing into discrete
     sign units. Boundary cues: hold, movement reset, non-manual reset.
   - **Sign recognition** — parameter classification (handshape /
     location / movement / non-manual) → gloss.
   - **Gloss → text** — gloss sequence to grammatical sentence in the
...
6. Define the canonical parameter schema for this pipeline. Anchor it
   to one of: **HamNoSys** (Hamburg Notation System — phonetic, machine
   readable), **SignWriting** (visual, human readable), or **SLPA**
   (Sign Language Phonological Annotation — for computational research).
...
7. The five non-negotiable parameters per sign:
   - **Handshape** — categorical, from the catalog's inventory (ASL
     has ~45 phonemic handshapes; TSL inventory differs).
   - **Location** — relative to body grid (neutral space, chest, chin,
     forehead, dominant-hand passive base, etc.).
   - **Movement** — primitives (straight, arc, circular, repeated;
     direction; size).
   - **Orientation** — palm direction + finger direction.
   - **Non-manual features** — brow position, eye gaze, head movement,
...
9. List the corpora for the target language. Examples (not exhaustive):
   - ASL: How2Sign, ASL-LEX, RWTH-PHOENIX (DGS — note the language),
     WLASL, MS-ASL.
...
12. For **text→sign** gloss generation: seq2seq transformer fine-tuned
    on parallel text↔gloss data; few-shot prompting against a frontier
    LLM only as a starting point with explicit eval.
13. For **sign→text** recognition: choose between continuous sign
    language recognition (CSLR) and isolated sign language recognition
    (ISLR). CSLR is the realistic target — most public-facing video
    is continuous.
...
15. Metrics for text→sign:
    ...
    - Native-signer fluency rating (Likert 1–5, with annotated
      disfluencies).
    - End-to-end intelligibility (Deaf user comprehension test).
16. Metrics for sign→text:
...
- **Code-switching** (mouthing English while signing ASL, fingerspelling
  inserted into signed sentences) — model must detect mode switches.
- **Regional / dialectal variation** within the same sign language (BSL
  Scotland vs London, ASL Black ASL, TSL regional dialects) — document
...
- **Classifier predicates** (depicting verbs in ASL, polymorphemic
  constructions) — recognition systems trained on lexicalized signs
  fail catastrophically here. Document the gap.
- **Non-manual-only signs** (some questions, some negations) — pure
  hand-tracking misses them. Non-manual features are mandatory.
- **Real-time captions over live signing** — latency budget tightens to
...
- **`/sign-linguistics`** — the catalog this pipeline consumes. Hard
  prerequisite (Step 1).
- **`/eval-store` (S9)** — persists eval runs and native-signer scores.
- **`/avatar-sign`** (Family P) — consumes the text→sign parameter
  sequence when the render target is an avatar.
- **`/scrape`** + **`markitdown`** — for ingesting linguistic
  documentation when the catalog needs expansion.
- **MediaPipe Holistic** — pose backbone default for sign→text.
- **Sapiens** (Meta) — quality-first pose backbone for offline.
- **Hall's prior sign-language work** (if applicable to TSL) — log
  citation in `.gstack/decisions/sign-{lang}.md`.

Primary linguistic sources to cite (examples — pick those relevant to
the target language):
...
5. **Direction-specific eval.** Text→sign and sign→text have different
