...
# /buy-vs-build — Vendor-vs-Build Scorecard — Family B — CTO Strategy
...
- `/buy-vs-build` — interactive: ask what capability, then run.
- `/buy-vs-build "<capability>"` — capability provided, skip the first question.
- `/buy-vs-build --vendors "A,B,C"` — vendor candidates pre-supplied, skip research.
...
- **Capability** — one sentence: "We need transactional email delivery for password resets and receipts."
- **Volume/scale today** — rough numbers (users, requests/sec, $ value moving through).
- **Volume/scale in 18 months** — best-faith projection.
- **Constraints** — compliance, data residency, latency, language SDK requirements.
- **Team capacity** — engineers available, current load.
- **Sunset cost tolerance** — what's the budget if we have to swap in 18 months?
...
- A) Yes, core differentiator — STOP. Buy is a bad framing; route to `/plan-eng-review` instead.
- B) No, table-stakes infrastructure — proceed.
- C) Hybrid (buy for now, build later as we differentiate) — proceed, note in the recommendation.
...
- 12+ months of activity (commits, releases, blog posts, status page incidents — any pulse).
...
For each candidate, score 1-5 on each dimension. Score honestly — a 5 means "best-in-class for this dimension, hard to beat" not "fine."
...
Show the table. Do NOT just emit the highest score — the recommendation must address WHY this candidate wins, not just that it does.
...
3. **The "boring" check**: is this the boring choice? If the leader is a flashy new entrant beating a battle-tested incumbent, raise the bar. "Boring by default" — three innovation tokens per company per year, don't spend one here unless the win is large and durable.
...
- **{Vendor A}** — {one-line description}. {URL or doc link}.
- **{Vendor B}** — {one-line description}. {URL or doc link}.
- **{Open-source X}** — {one-line description}. {repo URL}.
- **Build it ourselves** — {what the smallest viable build looks like, in eng-months}.
...
- A) Yes, write ADR — invoke `/adr` with title `Use {winner} for {capability}`, linking back to this doc.
- B) Not yet — Hall reviews the recommendation first.
- C) No, skip ADR — buy-vs-build is the artifact.
...
- `decisions/buy-vs-build/{slug}.md` — the full scorecard doc.
- Optional `docs/adr/{NNNN}-*.md` (if Step 7 = A).
- `intel.jsonl` entry.

## Edge cases

- **Hall wants to buy something that's a moat** — refuse at Step 0; route to `/plan-eng-review`.
- **Only one vendor exists** — flag as a risk (single-supplier). Still produce the scorecard with "Build" as the alternative. Recommend the buy if the single supplier passes risk muster, but explicitly note the lock-in.
- **No vendor exists, only build** — write a one-page "Build is the only viable path" doc, route to `/plan-eng-review` to scope the build.
- **Build wins on score but loses on time-to-value** — present a hybrid recommendation: "Buy for now, build the abstraction layer that lets us swap in 18 months."
- **Hall passes `--vendors` but none of them pass the candidate bar** — score them anyway, note the issue in the doc, recommend `/discover` for a broader sweep.

## Dependencies

- **S1 (decision store)** — output path `decisions/buy-vs-build/` is part of the shared CTO-strategy decision store.
- **S4 (decision store cross-reference)** — readable by `/adr`, `/vendor-score`, `/tech-debt-register`.
- **S7 (intel schema)** — log entries use `type: "buy_vs_build"`.
- **`/discover`** (shipped) — vendor research when `--vendors` not supplied.
- **`/adr`** — chained at Step 7 to record the decision formally.
- No new libraries.
