...
### Merged upstream gstack v1.44.0.0 → v1.55.0.0 (12 upstream releases)

Fork-sync release. Pulled garrytan/gstack `v1.44.1.0` through `v1.55.0.0` into hstack and re-anchored hstack on top. No hstack features were removed or changed.

#### What's new from upstream

- v1.55.0.0 — gbrain data-loss guards: `/sync-gbrain` can no longer trigger autopilot to `rm -rf` a working tree (feature-detected guards, realpath canonicalization, `--allow-reclone` gate); headed-browser crash-loop fixed; big-brain ingests no longer killed at 30 min; installer moved off the frozen v0.18.2 pin
- v1.54.0.0 — `/ship` carved into a 69KB skeleton + on-demand `sections/*.md` (-59% always-loaded); version-bump logic moved to the tested `gstack-version-bump` CLI
- v1.53.1.0 — non-interactive-safe plan-tune hook install (flag/env/config-driven, 10s auto-skip; no more hangs in Conductor/forwarded terminals)
- v1.53.0.0 — unified redaction engine (`lib/redact-patterns.ts` + `gstack-redact`) guards `/spec`, `/ship`, `/cso`, `/document-*` at every egress sink
- v1.52.2.0 — make-pdf emoji render on Linux (font fallback + `./setup` installs `fonts-noto-color-emoji`)
- v1.52.1.0 — brain-aware planning: 5 planning skills preflight a typed gbrain entity model before the first AskUserQuestion
- v1.52.0.0 — `/plan-tune` hooks made real: PostToolUse capture, PreToolUse auto-decide substitution, free-text "Other" loops back as structured memory
- v1.51.0.0 — `$B memory` diagnostic + 4 CDP-resource leak fixes in the browse server (with tripwire tests)
- v1.48.0.0 — AskUserQuestion 5+ option split rule + runtime AUTO_DECIDE carve-out (Conductor's 4-option cap is split-or-batch, not a silent trim)
- v1.47.0.0 — `/spec` skill: vague intent → executable spec in 5 phases, optional agent spawn, dedupe vs issues, `/ship` auto-closes the source issue
- v1.46.0.0 — gstack v2 foundation: catalog tokens -56%, eval-first floor covers all 51 skills, hard token + dollar caps gate every PR
- v1.45.0.0 — persistent design board daemon: 24h boards, one tab, board history
- v1.44.1.0 — fix wave: 9 community fixes (office-hours counter, iOS QA tunnels on macOS 26.x, Windows brain-sync, browse bind diagnostics)

See entries [1.55.0.0] through [1.44.1.0] below for full upstream changelogs.

#### Preserved hstack features

All hstack features carried forward unchanged: `/feature-build` family, UX Pipeline (v3.1) family
(`/product-ci`, `/ux-workflows`, `/mockup`, `/interactive-mockup`, `/ux-pipeline`), UX audit family,
`/plan-ux-review`, `/discover`, `/intel`, `/verify-loop`, `/investigate-workflow`, `/autoplan-full`,
`/autobuild`, `/autoaudit`, `/check-ci`, `/check-deps`, `/check-issues`, hstack identity in
`CLAUDE.md`/`README.md`, shared persona registry, all hstack skills. The hstack `/learn` and
`/autoplan` README rows kept their hstack wording; upstream's new `/spec` row and review-picker
table were added without duplicating them.

#### Conflict resolutions

- `VERSION` + `package.json` → `1.55.0.1` (hstack patch on top of upstream `1.55.0.0`)
- `CHANGELOG.md` → new merge entry on top; upstream `[1.55.0.0]`–`[1.44.1.0]` below; prior hstack `[1.44.0.1]` merge entry preserved below upstream's run
- `README.md` → added upstream's `/spec` row + "Which review should I use?" table; dropped upstream's duplicate `/autoplan` and `/learn` rows (hstack already lists them with hstack wording)
- 46 generated `SKILL.md` files → regenerated from auto-merged `.tmpl` templates via `bun run gen:skill-docs` (never hand-resolved)
- `gstack/llms.txt` → regenerated post-merge
- auto-merged cleanly: `AGENTS.md`, `CLAUDE.md`, all `SKILL.md.tmpl` templates, `scripts/resolvers/index.ts`, `browse/src/write-commands.ts`, `docs/skills.md`

The current head is `1.55.0.1`.
...
### Merged upstream gstack v1.39.2.0 → v1.44.0.0 (11 upstream releases)

Fork-sync release. Pulled garrytan/gstack `v1.39.2.0` through `v1.44.0.0` into hstack and re-anchored hstack on top. No hstack features were removed or changed.

#### What's new from upstream

- v1.44.0.0 — long-lived sidebar: WebSocket keepalive, transparent re-attach across network blips with scrollback intact, restart button that actually kills the old claude before spawning the new one, outer supervisor opt-in
- v1.43.3.0 — fix wave: headed-mode idle timer + `onDisconnect` target wrong `BrowserManager` for embedders
- v1.43.2.0 — fix wave: 18 post-Daegu paper-cuts (28 bisect commits)
- v1.43.1.0 — default PGLite to voyage-code-3 for code search + e2e tests
- v1.43.0.0 — iOS device-farm: 5 skills, Mac daemon, Tailscale tunneling
- v1.42.2.0 — fix wave: browse launch hardening (2 fixes + headed exit-code wiring)
- v1.42.1.0 — gate terminal-agent teardown on `ServerConfig.ownsTerminalAgent` (unblocks gbrowser embedder)
- v1.42.0.0 — Daegu wave: 23 community-filed bugs + PTY classifier enforcement (24 bisect commits)
- v1.41.1.0 — fix wave: 7 HIGH bugs from external audit + regression tests (PR #1169 follow-up)
- v1.40.0.0 — fix wave: gbrain sync hardening (8 community PRs + migration)
- v1.39.2.0 — `GSTACK_*` env-shim for Conductor + gbrain/gstack setup docs

See entries [1.44.0.0] through [1.39.2.0] below for full upstream changelogs.
...
`/plan-ux-review`, `/discover`, `/intel`, `/verify-loop`, `/investigate-workflow`, `/autoplan-full`,
`/autobuild`, `/autoaudit`, `/check-ci`, `/check-deps`, `/check-issues`, hstack identity in
`CLAUDE.md`/`README.md`, shared persona registry, all 178 hstack skills.

#### Conflict resolutions

- `VERSION` + `package.json` → `1.44.0.1` (hstack patch on top of upstream `1.44.0.0`)
- `CHANGELOG.md` → new merge entry on top; upstream `[1.44.0.0]`–`[1.39.2.0]` interleaved below; prior hstack `[1.41.0.0]`, `[1.40.0.0]`, `[1.39.1.1]` entries preserved below upstream's run
- `docs/skills.md` → merged additive rows from both sides
- `gstack/llms.txt` → merged both sides
- auto-merged cleanly: `AGENTS.md`, `CLAUDE.md`, `README.md`, `browse/src/write-commands.ts`, ~15 SKILL.md files under shared skill directories

#### Version note

Both hstack and upstream independently shipped a `[1.40.0.0]` entry — hstack's described "v3 launches: 107 new skills" (2026-05-16) while upstream's described "gbrain sync hardening" (2026-05-19). Per the gstack monotonic-ordering invariant ("queue-advancing past a claimed version within the same bump level is explicitly permitted"), both entries remain in CHANGELOG.md as historical artifacts. The current head is `1.44.0.1`.
...
Fork-sync release. Pulled garrytan/gstack `v1.28.0.0` through `v1.39.1.0` into hstack and re-anchored hstack on top. No hstack features were removed or changed.

#### What's new from upstream
- v1.39.1.0 — EXIT PLAN MODE GATE for plan-mode review skills (blocking gate before ExitPlanMode)
- v1.39.0.0 — `buildFetchHandler` factory unblocks gbrowser submodule consumption
- v1.38.1.0 — fix wave: surrogate-safe page captures + Implementation Tasks across review skills + root-level artifact patterns
- v1.38.0.0 — fix wave: Windows install hardening + Unicode sanitization at server egress (4 community PRs)
- v1.37.0.0 — split-engine gbrain: remote MCP brain + local PGLite for code
- v1.35.0.0 — `/document-generate` skill + enhanced `/document-release` with Diataxis coverage map
- v1.34.2.0 — fix wave: `/codex` review on CLI 0.130+, `/investigate` learnings, `/sync-gbrain` on Supabase (3 community-reported bugs)
- v1.34.1.0 — `gstack-update-check` resists stale GitHub raw CDN + adds semver-order guard
- v1.34.0.0 — gstack consumable as submodule (factory-export API + AUTH_TOKEN env + import.meta.main gate)
- v1.33.2.0 — setup guards against Conductor worktree pollution of global install
- v1.33.1.0 — learnings: token-OR query + task-shaped retrieval in 3 long skills
- v1.33.0.0 — `/sync-gbrain` memory-stage batch-import refactor (D1-D8) + F6/F9 + signal cleanup
- v1.32.0.0 — fix wave: 7 community PRs + 5 gate-eval hardenings
- v1.31.1.0 — fix wave: 3 community PRs (careful BSD sed, codex Step 0 rename, make-pdf setup ordering)
- v1.31.0.0 — delete AskUserQuestion fallback (root cause of forever war) + harness primitives
- v1.30.0.0 — fix wave: 21 community PRs + Windows CI extension + codex flag-semantics smoke
- v1.29.0.0 — worktree-aware gbrain code sources via path-hash IDs and CWD pin
- v1.28.0.0 — `browse --headed`/`--proxy`/`--navigate` + `gstack/llms.txt` + webdriver-only stealth

See entries [1.39.1.0] through [1.28.0.0] below for full upstream changelogs.

#### Preserved hstack features
All hstack features carried forward unchanged: `/feature-build` family, UX audit family,
`/plan-ux-review`, `/discover`, `/intel`, `/verify-loop`, `/investigate-workflow`,
`/autoplan-full`, `/autobuild`, `/autoaudit`, `/check-ci`, `/check-deps`, `/check-issues`,
hstack identity in `CLAUDE.md`/`README.md`, shared persona registry.

#### Conflict resolutions
- `VERSION` + `package.json` → `1.39.1.1` (hstack patch on top of upstream `1.39.1.0`)
- `CHANGELOG.md` → new merge entry on top, upstream `[1.39.1.0]`–`[1.28.0.0]` interleaved, prior hstack `[1.27.1.1]` entry preserved below upstream's run
- `README.md` → kept hstack-branded skill table + identity sections; merged upstream's additive rows
- auto-merged cleanly: `CLAUDE.md`, `AGENTS.md`, `browse/src/write-commands.ts`, `cso/SKILL.md`, `design-review/SKILL.md`, `docs/skills.md`, `qa/SKILL.md`, `qa/SKILL.md.tmpl`, `review/SKILL.md`, `scripts/resolvers/index.ts`, `test/hook-scripts.test.ts`
...
### Merged upstream gstack v1.21.1.0 → v1.27.1.0 (12 upstream releases)

Fork-sync release. Pulled garrytan/gstack `v1.21.1.0` through `v1.27.1.0` into hstack and re-anchored hstack on top. No hstack features were removed or changed.

#### What's new from upstream
- v1.27.1.0 — anti-shortcut clause + gate-tier AskUserQuestion floor tests for all `/plan-*` skills
- v1.27.0.0 — `/setup-gbrain` Path 4 (remote MCP) + brain → artifacts rename
- v1.26.5.0 — gbrain ingest writer (hybrid frontmatter) + gbrain-valid source ids
- v1.26.4.0 — GSTACK REVIEW REPORT delete-then-append (no more mid-file leftovers)
- v1.26.3.0 — `/sync-gbrain` skill + native code-surface orchestrator
- v1.26.2.0 — `/plan-eng-review` STOP gates always fire AskUserQuestion + report-at-bottom contract enforcement
- v1.26.1.0 — gbrain-sync orchestrator resolves sibling via `import.meta.dir`
- v1.26.0.0 — V1 transcript ingest + per-skill gbrain manifests + retrieval surface
- v1.25.1.0 — `/office-hours` Phase 4 STOP gate + AskUserQuestion recommendation judge
- v1.25.0.0 — AskUserQuestion resolves to host MCP variant when native is disallowed
- v1.24.0.0 — cross-platform hardening: curated Windows lane + `Bun.which` resolver + path-portability helper
- v1.23.0.0 — always prefix PR titles with `v<VERSION>`
- v1.21.1.0 — tighten `/plan-ceo-review` smoke (Step 0 must fire)

See entries [1.27.1.0] through [1.21.1.0] below for full upstream changelogs.
...
#### Conflict resolutions
- `VERSION` + `package.json` → `1.27.1.1` (hstack patch on top of upstream `1.27.1.0`)
- `CHANGELOG.md` → new merge entry on top, upstream `[1.27.1.0]`–`[1.21.1.0]` interleaved, prior hstack `[1.20.0.1]` entry preserved below upstream's run
- `README.md` → kept hstack-branded skill table + identity sections; merged upstream's `/sync-gbrain` row additively
- `cso/`, `design-review/`, `qa/`, `review/` SKILL.md → auto-merged cleanly; regenerated via `bun run gen:skill-docs`
- `scripts/resolvers/index.ts` → auto-merged cleanly
...
### Merged upstream gstack v1.5.2.0 → v1.20.0.0 (15 upstream releases)

This is a fork-sync release. We pulled garrytan/gstack `v1.5.2.0` through `v1.20.0.0`
into hstack and re-anchored hstack on top. No hstack features were removed.

#### What's new from upstream
- v1.20.0.0 — browser-skills runtime + `/scrape` / `/skillify` (deterministic Playwright scripts; ~200ms after first prototype)
- v1.17.0.0 — `/setup-gbrain` wireup + gbrain federation surface
- v1.16.0.0 — tunnel allowlist 17→26 + canDispatchOverTunnel pure function
- v1.15.0.0 — slim preamble + real-PTY plan-mode E2E harness
- v1.14.0.0 — gstack browser sidebar = interactive Claude Code REPL with live tab awareness
- v1.13.0.0 — Claude outside-voice skill
- v1.12.x  — `/setup-gbrain` for coding-agent onboarding + day-two fixes
- v1.11.x  — workspace-aware `/ship` version allocation + plan-mode handshake fix
- v1.10.x  — overlay efficacy harness + `AskUserQuestion` cadence + Pros/Cons format
- v1.9.0.0  — `gbrain-sync` cross-machine gstack memory
- v1.6.x    — Haiku classifier FP cut 44%→23%; tunnel dual-listener + SSRF + envelope + path security wave; RECOMMENDATION + Completeness split + Codex ELI10 restore
- v1.5.2.0 (upstream) — Opus 4.7 migration: model overlay, voice, routing

See entries [1.20.0.0] through [1.6.0.0] below for full upstream changelogs.

#### Preserved hstack features
- `/feature-build` family (`/feature-build`, `/auto-feature-build`, `/auto-feature-build-full`, `/discover-personas`, `/define-workflows`, `/gap-analysis`)
- UX audit family (`/ux-audit`, `/auto-ux-audit`, `/auto-ux-audit-full`)
- `/plan-ux-review`, `/discover`, `/intel`, `/verify-loop`, `/investigate-workflow`
- `/autoplan-full` (with Phase 4.5 DX review for developer-facing plans), `/autobuild`, `/autoaudit`
- `/check-ci`, `/check-deps`, `/check-issues`
- hstack project identity section in `CLAUDE.md`
- hstack tables + pipelines in `README.md`
- Shared persona registry pattern at `.gstack/ux-audit/personas.md`

#### Conflict resolutions
- `VERSION` and `package.json` → `1.20.0.1` (hstack patch on top of upstream `1.20.0.0`)
- `CHANGELOG.md` → new merge entry on top, upstream's [1.20.0.0]–[1.6.0.0] interleaved as-is, prior hstack [1.5.2.0] entry preserved below upstream's run
- `README.md` → kept hstack-branded sections; merged upstream's new docs additively
- `CLAUDE.md` → auto-merged cleanly (project identity section preserved at top, upstream additions appended)

#### Note on prior `[1.5.2.0]` hstack entry
The prior hstack release was tagged `[1.5.2.0]` while upstream was at `1.5.1.0`.
Per the documented `X.Y.Z.N` scheme that should have been `[1.5.1.1]`. The historical
entry is kept verbatim under the original `[1.5.2.0]` heading to preserve git history;
the next hstack-only release should reset to the corrected scheme.
