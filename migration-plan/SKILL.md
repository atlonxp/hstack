...
version: 1.0.0
description: |
  [Chief Architect] Given a from→to migration (DB engine swap, framework upgrade,
  language port, region move, monolith→services), picks the safest pattern
  (strangler fig, parallel run, blue-green, canary, dark launch) and emits a
  phased plan with cutover criteria, rollback triggers, observability
  requirements, and team-capacity check. Also produces an `/autobuild`-compatible
  ...
  or "how do we get off this thing". (gstack)
allowed-tools:
  - Read
  - Write
  - Edit
