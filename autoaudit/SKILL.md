...
name: autoaudit
version: 1.0.0
description: "[Code Review] Post-build verification pipeline for gstack — runs security audit (/cso) then code review (/review + /codex) sequentially."
...
Reports findings, auto-fixes obvious issues,
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"autoaudit","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"autoaudit","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
