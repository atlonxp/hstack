...
preamble-tier: 2
version: 1.0.0
description: "[Build & Autonomy] Self-healing verification loop for gstack."
...
Runs /investigate-workflow logic on built features,
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"verify-loop","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"verify-loop","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Read the `/autobuild` skill file at `~/.claude/skills/gstack/autobuild/SKILL.md` using the Read tool.

**If unreadable:** Skip with "Could not load /autobuild — skipping." and continue.
