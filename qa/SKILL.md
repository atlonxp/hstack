...
preamble-tier: 4
version: 2.0.0
description: "[QA & Bugs] Systematically QA test a web application and fix bugs found. (gstack)"
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
~/.claude/skills/gstack/bin/gstack-intel-append '{"ts":"'\$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"qa","event":"finding","severity":"SEVERITY","area":"AREA","file":"FILE_PATH","commit":"'\$(git rev-parse --short HEAD 2>/dev/null || echo "unknown")'"}' &
```
