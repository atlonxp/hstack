...
describe('Codex generation (--host codex)', () => {
  ...
  test('no ~/.claude/ paths in Codex output', () => {
    // /auto-guard installs Claude Code hooks, so it documents Claude Code's own
    // settings location (~/.claude/settings.json) as a literal — that path is
    // correct regardless of host, not a host-portability leak. Exempt it.
    const CLAUDE_PATH_OK = new Set(['gstack-auto-guard']);
    for (const skill of CODEX_SKILLS) {
      if (CLAUDE_PATH_OK.has(skill.codexName)) continue;
      const content = fs.readFileSync(path.join(AGENTS_DIR, skill.codexName, 'SKILL.md'), 'utf-8');
