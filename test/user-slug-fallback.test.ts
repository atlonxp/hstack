...
describe('resolve-user-slug fallback chain', () => {
  ...
  test('persists resolution to user_slug_at_<hash> on first call', () => {
    ...
    // endpoint_hash() returns sha8 hex when a gbrain MCP URL is configured in
    // ~/.claude.json, else the literal "local" (the no-MCP fallback, and the
    // clean-CI case). Accept both so the gate doesn't require a gbrain endpoint.
    expect(content).toMatch(/^user_slug_at_(?:[a-f0-9]+|local):\s+persisttest/m);
  });
