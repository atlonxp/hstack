 ...
 *
 * Asserts that no skill's generated SKILL.md grew beyond the v1.55.0.1
 * baseline. Catches preamble/resolver changes that bloat skills back to
 * the pre-compression size. Free — pure file IO + JSON diff.
 *
 * Baseline rebased v1.44.1 → v1.55.0.1 in the AskUserQuestion split-rule
 ...
 *
 * The baseline lives at test/fixtures/parity-baseline-v1.55.0.1.json,
...
const BASELINE_PATH = path.join(REPO_ROOT, 'test', 'fixtures', 'parity-baseline-v1.55.0.1.json');
...
describe('SKILL.md size budget regression (gate, free)', () => {
  test('parity-baseline-v1.55.0.1.json exists', () => {
    expect(fs.existsSync(BASELINE_PATH)).toBe(true);
  });

  test('no skill exceeds v1.55.0.1 baseline size × ratio', () => {
    ...
    throw new Error(
      `${regressions.length} skill(s) regressed past v1.55.0.1 baseline × ${RATIO}:\n${msg}\n` +
  ...
  test('total corpus byte count does not regress past baseline × ratio', () => {
    ...
    throw new Error(
      `Total corpus regressed past v1.55.0.1 baseline × ${RATIO}: ` +
   ...
   * a tiny noise floor. A skill that was 100 KB at v1.55.0.1 and shrinks to
   ...
   * shrinkage floor.
   *
   * Floor: 80% of the v1.55.0.1 baseline. v1.46 actual shrinkage is <1% per
   ...
   * relaxes for them.
   */
  test('no skill shrinks past 80% of v1.55.0.1 baseline (catches accidental body strip)', () => {
    const baseline: ParityBaseline = JSON.parse(fs.readFileSync(BASELINE_PATH, 'utf-8'));
    const current = captureBaseline({ repoRoot: REPO_ROOT });
    ...
    throw new Error(
      `${undershoots.length} skill(s) shrunk past v1.55.0.1 × ${MIN_RATIO} floor:\n${msg}\n` +
  ...
  test('catalog token estimate stays compressed (v1.45 target ≤ 7000)', () => {
    const current = captureBaseline({ repoRoot: REPO_ROOT });
    const v145Target = 14000; // rebased v1.55.0.1: hstack 182-skill catalog ~12696 tokens + ~10% regression headroom
