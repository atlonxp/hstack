...
These skills ship in hstack on top of upstream gstack. See [README.md](../README.md) for the hstack identity and persona-aware pipelines.

| Skill | Your specialist | What they do |
|-------|----------------|--------------|
| [`/discover`](#discover) | **CTO** | Product/tech intelligence — competitor scans, emerging tech, market gaps, opinionated recommendations. |
| [`/discover-personas`](#discover-personas) | **UX Researcher** | Discover and document the personas this app serves. Foundation for downstream persona-aware pipelines. |
| [`/define-workflows`](#define-workflows) | **UX Researcher** | Map each persona's end-to-end workflows before any feature work begins. |
| [`/gap-analysis`](#gap-analysis) | **Product Manager** | Per-persona feature gap analysis against the workflows defined for that persona. |
| [`/feature-build`](#feature-build) | **Builder** | Build one feature with persona-aware planning + verification + audit. |
| [`/auto-feature-build`](#auto-feature-build) | **Builder (single persona)** | Auto pipeline: gap analysis → autoplan → autobuild → audit for one persona. |
| [`/auto-feature-build-full`](#auto-feature-build-full) | **Builder (all personas)** | Full cross-persona gap fill: dedup features, build in dependency order, cross-persona regression check. |
| [`/ux-audit`](#ux-audit) | **UX Auditor** | Persona-driven UX workflow audit on a live site. Reports where each persona gets stuck. |
| [`/auto-ux-audit`](#auto-ux-audit) | **UX Auditor + Fixer (single persona)** | Audit + fix loop for one persona with atomic commits and browser re-verification. |
| [`/auto-ux-audit-full`](#auto-ux-audit-full) | **UX Auditor + Fixer (all personas)** | All personas with cross-persona consistency checks between rounds. |
| [`/plan-ux-review`](#plan-ux-review) | **UX Researcher** | Plan-mode UX review: stakeholder workflows, persona journeys, handoff points, cross-feature flows. |
| [`/intel`](#intel) | **Project Intelligence** | Cross-session memory briefing — recurring patterns, recent findings, known hotspots. |
| [`/investigate-workflow`](#investigate-workflow) | **Workflow Investigator** | Post-implementation completeness check. Walks every code path end-to-end. |
| [`/verify-loop`](#verify-loop) | **Verifier** | Post-build verification loop that re-checks workflows after each fix. |
| [`/autoplan-full`](#autoplan-full) | **Full Planner** | office-hours → discover → CEO → UX → DX → design → eng review. From idea to fully reviewed plan. |
| [`/autobuild`](#autobuild) | **Autonomous Builder** | Autonomous implementation from an approved plan. Headless or minimal supervision. |
| [`/autoaudit`](#autoaudit) | **Auditor** | Post-build audit: security review (/cso) + code review (/review + /codex). |
| [`/check-ci`](#check-ci) | **CI Reporter** | One-shot CI status check. Surfaces failing workflows and suggests fixes. |
| [`/check-deps`](#check-deps) | **Dependency Auditor** | Dependency staleness + vulnerability scan. |
| [`/check-issues`](#check-issues) | **Triage** | Open issues snapshot for the current repo. |
