...
preamble-tier: 2
version: 1.0.0
description: "[Economic Simulation] Designs a real-world event ingestion pipeline for an economic simulator with observability-time modeling — agents react to events when the information would have been... (gstack)"
allowed-tools:
  - Bash
  - Read
  - Write
...
per run. Pairs with `/econ-sim`, `/counterfactual`, and `/sim-calibrate`.
...
event modeling", or "replay vs live event ingestion".
