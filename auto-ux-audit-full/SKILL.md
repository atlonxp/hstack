...
preamble-tier: 4
version: 1.0.0
description: "[UX Audit] Full UX audit across all personas with auto-fix. (gstack)"
allowed-tools:
  - Bash
  - Read
  - Write
...
For a single persona, use /auto-ux-audit. For report-only, use /ux-audit.
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"auto-ux-audit-full","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"auto-ux-audit-full","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
