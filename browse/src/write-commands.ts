...
export async function handleWriteCommand(
  ...
  switch (command) {
    case 'goto': {
      ...
      const response = await page.goto(normalizedUrl, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT });
    ...
    case 'wait': {
      ...
      if (selector === '--networkidle') {
        ...
        const timeout = Math.min(Math.max(args[1] ? parseInt(args[1], 10) || MIN_WAIT_MS : NAV_TIMEOUT, MIN_WAIT_MS), MAX_WAIT_MS);
